...
async def run_celery_sequential():
    ...
    for idx, (comp_name, fname) in enumerate(target_files, 1):
        print(f"\n------------------------------------------------------------------------")
        print(f"[{idx}/8] TRIGGERING CELERY TASK FOR: '{fname}' ({comp_name})...")
        print(f"------------------------------------------------------------------------")
        ...
        # Trigger task in Celery worker container
        async_res = process_document_task.delay(doc_id)
        print(f"Task sent to Celery worker! Task ID: {async_res.id}. Waiting for completion...")
        ...
        while True:
            ...
            print(f"  [{elapsed:.0f}s] Document status in finiq_db: {db_status} | Saved Chunks: {db_chunk_count}")
    ...
    print("\n================ FINAL SEQUENTIAL CELERY INGESTION SUMMARY ================")
...
if __name__ == "__main__":
    asyncio.run(run_celery_sequential())
