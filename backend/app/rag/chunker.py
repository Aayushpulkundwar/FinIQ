...
def should_discard_chunk(text: str) -> Tuple[bool, str]:
    ...
    Excludes structured tables and financial statements from aggressive unique word ratio filtering.
    """
    text_clean = text.strip()
    if not text_clean:
        return True, "Empty text"

    # Do not discard table chunks containing Markdown dividers or table headers
    if "|" in text_clean and ("---" in text_clean or text_clean.count("|") >= 4):
        return False, ""

    # 1. Unique word ratio (skip if text contains numbers / financial figures)
    words = [w.strip(".,;:!?()[]{}'\"").lower() for w in text_clean.split() if w.strip()]
    if words:
        unique_ratio = len(set(words)) / len(words)
        # Only discard non-numeric text with extremely repetitive words
        has_financial_numbers = any(re.search(r"\b\d+[\d,.]*\b", w) for w in words)
        if len(words) >= 15 and unique_ratio < 0.20 and not has_financial_numbers:
    ...
    if lines:
        ...
        if len(lines) >= 8 and avg_words < 1.2:
    ...
    if lines:
        ...
        if len(lines) >= 5 and toc_ratio > 0.70:
...
class DocumentChunker:
    ...
    def chunk_document(
        ...
        for page in pages:
            ...
            cleaned_lines = [l for l in lines if l.strip() not in repeating_lines]
        ...
        for item in page_chunks:
            ...
            if discard:
                logger.warning(
                    f"DocumentChunker: Discarded chunk doc_id={document_id}, page={page_number}, reason='{reason}'"
                )
                continue
        ...
        # Fallback: if all chunks were discarded but page blocks existed, create raw block chunks
        if not chunks and all_blocks:
            logger.warning(f"DocumentChunker: All chunks were discarded for doc_id={document_id}. Applying raw block fallback.")
            for idx, item in enumerate(all_blocks):
                b_text = item["text"].strip()
                if not b_text:
                    continue
                norm_text = re.sub(r"\s+", "", b_text.lower())
                chunk_hash = hashlib.md5(norm_text.encode("utf-8")).hexdigest()
                chunks.append({
                    "document_id": document_id,
                    "chunk_text": b_text,
        ...
        logger.info(f"DocumentChunker: Generated {len(chunks)} chunks for document {document_id}.")
        return chunks
